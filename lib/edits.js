...
// 정본에서 다시 제공하기로 한 앱이 브라우저의 과거 삭제 기록 때문에 계속 숨는 경우,
// 지정한 마이그레이션을 브라우저당 한 번만 적용한다. 이후 사용자가 다시 삭제하면 존중한다.
export function restoreDeletedReposOnce(state, migrationId, repos) {
  if (typeof window === "undefined") return state;
  const migrationKey = `deeptact-apps-migration-${migrationId}`;
  if (localStorage.getItem(migrationKey)) return state;

  const restoreSet = new Set(repos);
  const nextDeleted = state.deleted.filter((repo) => !restoreSet.has(repo));
  if (nextDeleted.length !== state.deleted.length) {
    saveDeleted(nextDeleted);
  }
  localStorage.setItem(migrationKey, "1");
  return { ...state, deleted: nextDeleted };
}
